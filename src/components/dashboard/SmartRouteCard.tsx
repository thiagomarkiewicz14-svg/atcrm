...
const badgeClassNames: Record<RoutePriorityBadge, string> = {
  ...
  OPORTUNIDADE: 'border-[#C8A951]/40 bg-[#C8A951]/20 text-[#7C5E3C]',
  RECUPERAR: 'border-[#1E3A2F]/20 bg-[#1E3A2F]/10 text-[#1E3A2F]',
};
  ...
  const content = (
    <div className="space-y-3">
      {suggestions.length === 0 ? (
        <div className="rounded-lg border-2 border-dashed border-[#1E3A2F]/20 bg-background p-4">
          <p className="text-sm font-bold text-[#1E3A2F]">Carteira está sob controle hoje.</p>
    ...
    <Card className="border-[#1E3A2F]/25 bg-[#F8F9F7]">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            <CardTitle className="text-[#1E3A2F]">ROTA INTELIGENTE HOJE</CardTitle>
            <p className="mt-2 text-sm font-medium text-muted-foreground">
              Prioridade sugerida para não perder janela no campo.
            </p>
          </div>
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border-2 border-[#1E3A2F]/20 bg-[#1E3A2F]/10 text-[#1E3A2F]">
...
function SmartRouteItem({ position, suggestion }: { position: number; suggestion: RouteSuggestion }) {
  ...
  return (
    <div
      ...
      className="cursor-pointer rounded-lg border-2 border-[#1E3A2F]/20 bg-background p-4 transition-all duration-150 hover:border-[#1E3A2F]/40 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1E3A2F]/30"
      aria-label={`Abrir cliente ${suggestion.clientName}`}
    >
      <div className="flex items-start gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#1E3A2F] text-sm font-black text-white">
          {position}
        </div>
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="space-y-1">
            <p className="text-sm font-bold text-[#1E3A2F]">{suggestion.reason}</p>
            <p className="line-clamp-2 text-xs font-medium text-muted-foreground">
              {suggestion.reasons.join(' · ')}
            </p>
          </div>

          <div className="flex flex-wrap items-center justify-between gap-3 border-t-2 border-[#1E3A2F]/10 pt-3">
            ...
            <Link
              ...
              className={cn(
                ...
                suggestion.action === 'start_visit'
                  ? 'bg-[#1E3A2F] text-white hover:bg-[#0F2D22]'
                  : 'border-2 border-[#1E3A2F] text-[#1E3A2F] hover:bg-[#1E3A2F] hover:text-white',
              )}
            >
              {actionLabel}
